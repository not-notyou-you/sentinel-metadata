...
class RegionOfInterest(Base):
    """Master AOI (Area of Interest) definitions."""
...
class ProcessingStage(Base):
    """Ordered ETL pipeline stage definitions."""
...
class SatelliteScene(Base):
    """Core registry of all Sentinel-1 SAR scenes."""
...
class ProcessingJob(Base):
    """ETL job execution log per scene × stage × attempt."""
...
class DataProduct(Base):
    """Output artifact registry (COG, filtered TIFF, etc.)."""
...
class QualityMetric(Base):
    """Per-scene per-band radiometric quality validation results."""
...
class ProcessingRule(Base):
    """Configurable ETL validation rules per pipeline stage."""
...
class DataLineage(Base):
    """DAG of product transformations for full provenance tracking."""
...
class ApiAccessLog(Base):
    """Full API audit trail (TimescaleDB hypertable)."""
...
class AlertEvent(Base):
    """Pipeline monitoring and quality alert events."""
...
class DatasetVersion(Base):
    """Semantic versioning for production dataset releases."""
...
class DatabaseClient:
    """
    Manages SQLAlchemy engine, connection pooling, and session lifecycle.

    Usage:
        db = DatabaseClient.from_env()
        with db.session() as session:
            scene = session.get(SatelliteScene, 1)
    """
    ...
    def __init__(self, database_url: str, pool_size: int = 5,
        ...
        Args:
            database_url : PostgreSQL DSN (postgresql+psycopg2://user:pass@host/db)
            pool_size    : Persistent connection pool size
            max_overflow : Additional connections allowed above pool_size
            echo         : Echo SQL statements to logger (dev only)
        """
    ...
    def from_env(cls) -> "DatabaseClient":
        ...
        Optional:
            DB_POOL_SIZE (default 5), DB_MAX_OVERFLOW (default 10), DB_ECHO (default false)
        """
    ...
    def session(self) -> Generator[Session, None, None]:
        """
        Provide a transactional database session with auto-commit/rollback.

        Yields:
            SQLAlchemy Session object

        Raises:
            SQLAlchemyError: On unrecoverable database errors (after rollback)
    ...
    def session_with_retry(self) -> Generator[Session, None, None]:
        """
        Session context manager with exponential-backoff retry for transient errors.

        Retries up to _RETRY_ATTEMPTS times on OperationalError (e.g. connection
        dropped, server restart). Non-transient errors (constraint violations, etc.)
        are not retried.
    ...
    def create_tables(self) -> None:
        """Create all tables (for plain PostgreSQL without migration files)."""
